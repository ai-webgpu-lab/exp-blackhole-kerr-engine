...
`과학형 블랙홀 엔진`를 단일 질문으로 분리해 검증하는 experiment 저장소입니다. 하나의 가설과 baseline에 집중하고, 결과를 재현 가능한 형식으로 남겨 이후 benchmark/app 저장소의 입력으로 사용합니다.
...
## 핵심 질문
- 과학형 블랙홀 엔진를 브라우저/WebGPU 환경에서 재현 가능한 baseline으로 만들 수 있는가
...
- Repo-specific Pages baseline copied from `repo-scaffolds/repos/exp-blackhole-kerr-engine/`
...
## 현재 baseline 상태
- Repository-specific runnable baseline active: Kerr geodesic engine readiness harness with capability probe, spin/inclination metadata, integration timing, and schema-aligned blackhole result export
- Generated override source: `repo-scaffolds/repos/exp-blackhole-kerr-engine/`
- Results/report scaffold is ready to promote exported JSON into `reports/raw/` and `RESULTS.md`

## GitHub Pages 운영 메모
- Pages URL: https://ai-webgpu-lab.github.io/exp-blackhole-kerr-engine/
...
- 이 저장소의 live demo는 `https://ai-webgpu-lab.github.io/exp-blackhole-kerr-engine/`입니다.
